...
async function hashData(entropyBuffer: Uint8Array): Promise<Uint8Array> {
    // The SubtleCrypto.digest() method will return a Promise that resolves to an ArrayBuffer
...
async function deriveChecksumBits(entropyBuffer: Uint8Array): Promise<string> {
    ...
    const CS = ENT / 32;
    const hash = await hashData(entropyBuffer)
    /* const hash = new Sha256()
      .update(entropyBuffer)
      .digest(); */
    //const hash = CryptoJS.SHA256(entropyBuffer).toString();
